...
func (s *convMsgRepository) UpdateConvWithMessage(ctx context.Context, msg *sdkws.MessageData) error {
...
func (s *convMsgRepository) tryUpdateConv(ctx context.Context, convID string, lastMessageSeq int64) (bool, error) {
	now := time.Now()
	store := s.getStore()
	...
	if err := store.UpdateOne(ctx, orm.CollectionConversations, filter, update); err == nil {
		// 更新成功（命中了旧 seq）
		klog.CtxDebugf(ctx, "[ConvMsgRepository] update existing conversation doc success",
			"conv_id", convID,
			"last_message_seq", lastMessageSeq)
		return true, nil
	} else if err != foundationstorage.ErrNotFound {
		// 真实错误，直接返回
		klog.CtxErrorf(ctx, "[ConvMsgRepository] update conversation doc failed",
			"conv_id", convID,
			"error", err.Error())
		return false, err
	}
...
func (s *convMsgRepository) tryInsertConv(ctx context.Context, conv *sdkws.ConversationData, lastMessageSeq int64) error {
	now := time.Now()
	store := s.getStore()
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConvMsgRepository] marshal conversation failed",
			"conv_id", conv.ConvID,
			"error", err.Error())
		return err
	}
	...
	if _, err := store.InsertOne(ctx, orm.CollectionConversations, doc); err != nil {
		if err != foundationstorage.ErrDuplicateKey {
			// 非重复键错误，直接返回
			klog.CtxErrorf(ctx, "[ConvMsgRepository] insert conversation doc failed",
				"conv_id", conv.ConvID,
				"error", err.Error())
			return err
		}
	...
	// 插入成功（第一次出现这个会话记录）
	klog.CtxDebugf(ctx, "[ConvMsgRepository] insert new conversation doc success",
		"conv_id", conv.ConvID,
		"last_message_seq", lastMessageSeq)
	return nil
}
...
func (s *convMsgRepository) tryUpdateConvAfterDuplicateInsert(ctx context.Context, convID string, lastMessageSeq int64) error {
	now := time.Now()
	store := s.getStore()
	...
	if err := store.UpdateOne(ctx, orm.CollectionConversations, filter, update); err != nil {
		...
		if err == foundationstorage.ErrNotFound {
			klog.CtxDebugf(ctx, "[ConvMsgRepository] skip outdated conversation update after duplicate insert",
				"conv_id", convID,
				"last_message_seq", lastMessageSeq)
			return nil
		}

		klog.CtxErrorf(ctx, "[ConvMsgRepository] update conversation doc failed after duplicate insert",
			"conv_id", convID,
			"error", err.Error())
		return err
	}

	klog.CtxDebugf(ctx, "[ConvMsgRepository] update existing conversation doc success after duplicate insert",
		"conv_id", convID,
		"last_message_seq", lastMessageSeq)
	return nil
}
