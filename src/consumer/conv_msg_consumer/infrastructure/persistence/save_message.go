...
func (s *convMsgRepository) SaveMessage(ctx context.Context, msgID string, msg *sdkws.MessageData) error {
	if msgID == "" || msg == nil {
		return nil
	}
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConvMsgRepository] marshal msg failed",
			"msg_id", msgID,
			"conv_id", msg.ConvID,
			"error", err.Error())
		return err
	...
	if err != nil {
		...
		klog.CtxErrorf(ctx, "[ConvMsgRepository] save message detail failed",
			"msg_id", msgID,
			"conv_id", msg.ConvID,
			"error", err.Error())
		return err
	...
	klog.CtxDebugf(ctx, "[ConvMsgRepository] save message detail success",
		"msg_id", msgID,
		"conv_id", msg.ConvID)
	return nil
}
