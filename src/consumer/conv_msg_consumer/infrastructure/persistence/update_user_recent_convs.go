...
func (s *convMsgRepository) UpdateUserRecentConversation(ctx context.Context, req *UpdateUserRecentConversationRequest) error {
...
func (s *convMsgRepository) tryUpdateExisting(ctx context.Context, doc *orm.UserRecentConversationsDocument) (bool, error) {
	store := s.getStore()
	...
	if err := store.UpdateOne(ctx, orm.CollectionUserRecentConversations, filter, update); err == nil {
		// 更新成功（命中了旧版本）
		klog.CtxDebugf(ctx, "[ConvMsgRepository] update existing user recent conversation doc success",
			"user_id", doc.UserID,
			"conv_id", doc.ConvID,
			"last_message_seq", doc.LastMessageSeq,
			"version", doc.Version)
		...
		// 真实错误，直接返回
		klog.CtxErrorf(ctx, "[ConvMsgRepository] update user recent conversation doc failed",
			"user_id", doc.UserID,
			"conv_id", doc.ConvID,
			"error", err.Error())
		return false, err
...
func (s *convMsgRepository) tryInsertNew(ctx context.Context, doc *orm.UserRecentConversationsDocument) error {
	store := s.getStore()

	if _, err := store.InsertOne(ctx, orm.CollectionUserRecentConversations, doc); err != nil {
		if err != foundationstorage.ErrDuplicateKey {
			// 非重复键错误，直接返回
			klog.CtxErrorf(ctx, "[ConvMsgRepository] insert user recent conversation doc failed",
				"user_id", doc.UserID,
				"conv_id", doc.ConvID,
				"error", err.Error())
			return err
	...
	klog.CtxDebugf(ctx, "[ConvMsgRepository] insert new user recent conversation doc success",
		"user_id", doc.UserID,
		"conv_id", doc.ConvID,
		"last_message_seq", doc.LastMessageSeq,
		"version", doc.Version)
...
func (s *convMsgRepository) tryUpdateUserRecentConvAfterDuplicateInsert(ctx context.Context, doc *orm.UserRecentConversationsDocument) error {
	store := s.getStore()
	...
	if err := store.UpdateOne(ctx, orm.CollectionUserRecentConversations, filter, update); err != nil {
		...
		if err == foundationstorage.ErrNotFound {
			klog.CtxDebugf(ctx, "[ConvMsgRepository] skip outdated user recent conversation update after duplicate insert",
				"user_id", doc.UserID,
				"conv_id", doc.ConvID,
				"last_message_seq", doc.LastMessageSeq,
				"version", doc.Version)
		...
		klog.CtxErrorf(ctx, "[ConvMsgRepository] update user recent conversation doc failed after duplicate insert",
			"user_id", doc.UserID,
			"conv_id", doc.ConvID,
			"error", err.Error())
		return err
	...
	klog.CtxDebugf(ctx, "[ConvMsgRepository] update existing user recent conversation doc success after duplicate insert",
		"user_id", doc.UserID,
		"conv_id", doc.ConvID,
		"last_message_seq", doc.LastMessageSeq,
		"version", doc.Version)
