...
func (s *consumerUsecase) pushMessage(ctx context.Context, msg *sdkws.MessageData) error {
	// 获取接收者列表
...
func (s *consumerUsecase) getPushTargets(msg *sdkws.MessageData) []string {
...
func (s *consumerUsecase) findMembersForConv(ctx context.Context, convID string) ([]string, error) {
	if convID == "" {
		return nil, nil
	}
	...
	conv, err := s.repo.GetConversationData(ctx, convID)
	if err != nil || conv == nil {
		return nil, err
	}
...
func (s *consumerUsecase) buildPushMessage(msg *sdkws.MessageData) *backbon.PushMessage {
	// 序列化消息数据
...
func (s *consumerUsecase) getBackbonServiceClient(ctx context.Context) (backbonservice.Client, error) {
