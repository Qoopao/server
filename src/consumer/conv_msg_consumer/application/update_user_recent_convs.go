...
func (s *consumerUsecase) updateUserRecentConvsIfSmallConv(ctx context.Context, msg *sdkws.MessageData) error {
	...
	for _, userID := range members {
		...
		// 更新最近会话链
		if err := s.repo.UpdateUserRecentConversation(ctx, &persistence.UpdateUserRecentConversationRequest{
			UserID:         userID,
			ConvID:         msg.ConvID,
...
// - 群聊：从会话文档中解析成员 JSON
// - 其他类型：返回空
func (s *consumerUsecase) getConvMembersForMessage(ctx context.Context, msg *sdkws.MessageData) ([]string, error) {
	switch msg.ConvType {
