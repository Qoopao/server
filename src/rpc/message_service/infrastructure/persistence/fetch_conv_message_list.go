...
func (s *messageRepository) FetchConvMessageListWithRange(ctx context.Context, convID string, left int64, right int64) ([]*sdkws.MessageData, error) {
	// 构建查询条件
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[MessageRepository] fetch conv message list failed",
			"conv_id", convID,
			"left", left,
			"right", right,
			"error", err.Error())
	...
	for _, doc := range docs {
		...
		if err := msg.Unmarshal(doc.Data); err != nil {
			klog.CtxErrorf(ctx, "[MessageRepository] unmarshal message data failed",
				"msg_id", doc.ID,
				"conv_id", doc.ConvID,
				"seq", doc.Seq,
				"error", err.Error())
	...
	klog.CtxDebugf(ctx, "[MessageRepository] fetch conv message list success",
		"conv_id", convID,
		"left", left,
		"right", right,
		"count", len(messages))
...
func (s *messageRepository) FetchConvLatestMessageList(ctx context.Context, convID string, limit int64) ([]*sdkws.MessageData, error) {
	// 构建查询条件
	filter := bson.M{"conv_id": convID}
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[MessageRepository] fetch conv latest messages failed",
			"conv_id", convID,
			"limit", limit,
			"error", err.Error())
		return nil, err
	...
	klog.CtxDebugf(ctx, "[MessageRepository] fetch conv latest messages success",
		"conv_id", convID,
		"limit", limit,
		"count", len(messages))
