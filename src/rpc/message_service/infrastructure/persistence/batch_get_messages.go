...
func (s *messageRepository) BatchGetMessages(ctx context.Context, messageIDs []string, convID string) (map[string]*sdkws.MessageData, error) {
	if len(messageIDs) == 0 {
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[MessageRepository] batch get messages failed",
	...
	klog.CtxDebugf(ctx, "[MessageRepository] batch get messages success",
