...
func (s *backUsecase) Call(ctx context.Context, req *back.CallRequest) (resp *back.CallResponse, err error) {
	// 创建响应
	...
	if stringutil.IsEmpty(serviceName) {
		...
		klog.CtxErrorf(ctx, "[BackUsecase] service name is empty")
		return resp, nil
	}

	if stringutil.IsEmpty(methodName) {
		resp.Error = "method name is empty"
		klog.CtxErrorf(ctx, "[BackUsecase] method name is empty")
		return resp, nil
	}
	...
	if callErr != nil {
		...
		klog.CtxErrorf(ctx, "[BackUsecase] call service failed - service: %s, method: %s, error: %v",
	...
	klog.CtxDebugf(ctx, "[BackUsecase] call service success - service: %s, method: %s",
...
func (s *backUsecase) callBackserviceIM(ctx context.Context, methodName string, payload []byte) ([]byte, error) {
	switch methodName {
