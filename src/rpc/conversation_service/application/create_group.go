...
func (s *conversationUsecase) CreateGroup(ctx context.Context, req *sdkws.CreateGroupRequest) (*sdkws.CreateGroupResponse, error) {
	// 1. 数据检查
	...
	// 3. 数据库写入
	if err := s.createConversationRepository(ctx, conv); err != nil {
		return &sdkws.CreateGroupResponse{ErrorCode: 500, ErrorMsg: err.Error()}, nil
	}
...
func (s *conversationUsecase) validateCreateGroupRequest(req *sdkws.CreateGroupRequest) (*sdkws.CreateGroupResponse, bool) {
...
func (s *conversationUsecase) buildGroupConversationData(ctx context.Context, req *sdkws.CreateGroupRequest) (*sdkws.ConversationData, error) {
	newID, err := uuid.NewRandom()
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] CreateGroup uuid.NewRandom failed: %v", err)
		return nil, err
	}
...
// createConversationRepository 将会话写入数据库；失败时在内部打错误日志并返回 error。
func (s *conversationUsecase) createConversationRepository(ctx context.Context, conv *sdkws.ConversationData) error {
	if err := s.repo.CreateConversation(ctx, conv); err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] CreateGroup repository.CreateConversation failed: conv_id=%s err=%v", conv.ConvID, err)
		return err
	}
	return nil
}
...
func (s *conversationUsecase) buildConversationCreatedCmd(ctx context.Context, conv *sdkws.ConversationData) *sdkws.CmdMessage {
	if conv == nil {
		return nil
	}
	data, err := conv.Marshal(nil)
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] buildConversationCreatedCmd marshal conv failed: conv_id=%s err=%v", conv.ConvID, err)
		return nil
	}
...
func (s *conversationUsecase) collectGroupPushTargets(req *sdkws.CreateGroupRequest) []string {
...
func (s *conversationUsecase) sendGroupCreatedSystemMessage(ctx context.Context, conv *sdkws.ConversationData, req *sdkws.CreateGroupRequest) {
	if conv == nil || req == nil {
		return
	}
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] sendGroupCreatedSystemMessage get message client failed: conv_id=%s err=%v", conv.ConvID, err)
		return
	}
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] sendGroupCreatedSystemMessage BatchSendMessage failed: conv_id=%s err=%v", conv.ConvID, err)
		return
	}
	if resp != nil && len(resp.Results) > 0 {
		r := resp.Results[0]
		if r.GetErrorCode() != "" {
			klog.CtxWarnf(ctx, "[ConversationUsecase] sendGroupCreatedSystemMessage business error: conv_id=%s code=%s msg=%s",
