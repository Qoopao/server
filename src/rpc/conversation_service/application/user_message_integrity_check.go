...
func (s *conversationUsecase) UserMessageIntegrityCheck(ctx context.Context, req *sdkws.UserMessageIntegrityCheckRequest) (*sdkws.UserMessageIntegrityCheckResponse, error) {
	...
	serverConvs, err := s.repo.FetchUserRecentConvListByVersionRange(ctx, userID, left, right)
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] UserMessageIntegrityCheck fetch recent convs failed",
			"user_id", userID,
			"left", left,
			"right", right,
			"error", err.Error())
	...
	klog.CtxDebugf(ctx, "[ConversationUsecase] UserMessageIntegrityCheck finished",
		"user_id", userID,
		"req_left", left,
		"req_right", right,
		"resp_left", respLeft,
