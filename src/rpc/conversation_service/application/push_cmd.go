...
func (s *conversationUsecase) pushCmdToUsers(ctx context.Context, userIDs []string, cmdMsg *sdkws.CmdMessage, requestID string) {
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] push cmd marshal CmdMessage failed: request_id=%s err=%v", requestID, err)
		return
	}

	client, err := s.serviceCtx.GetBackbonServiceClient(ctx)
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] push cmd get backbon client failed: request_id=%s err=%v", requestID, err)
		return
	}
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationUsecase] push cmd failed: request_id=%s err=%v", requestID, err)
		return
	}
	if resp.GetFailCount() > 0 {
		klog.CtxWarnf(ctx, "[ConversationUsecase] push cmd partial fail: request_id=%s success=%d fail=%d",
			requestID, resp.GetSuccessCount(), resp.GetFailCount())
	} else {
		klog.CtxDebugf(ctx, "[ConversationUsecase] push cmd success: request_id=%s count=%d", requestID, len(userIDs))
	}
}
