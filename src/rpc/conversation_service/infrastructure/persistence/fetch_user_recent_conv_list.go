...
func (s *conversationRepository) FetchUserRecentConvListByVersion(ctx context.Context, userID string, version int64) ([]*sdkws.ConversationData, error) {
	if userID == "" {
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationRepository] fetch user recent conv list by version failed",
			"user_id", userID,
			"version", version,
			"error", err.Error())
		return nil, err
	...
	for _, userConvDoc := range userConvDocs {
		...
		if !exists {
			klog.CtxWarnf(ctx, "[ConversationRepository] conversation not found",
				"user_id", userID,
	...
	klog.CtxDebugf(ctx, "[ConversationRepository] fetch user recent conv list by version success",
		"user_id", userID,
		"version", version,
...
func (s *conversationRepository) FetchUserRecentConvListByVersionRange(ctx context.Context, userID string, lowVersion int64, upVersion int64) ([]*sdkws.ConversationData, error) {
	if userID == "" {
		return nil, errors.New("user_id is empty")
	}
	...
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationRepository] fetch user recent conv list by version range failed",
			"user_id", userID,
	...
	klog.CtxDebugf(ctx, "[ConversationRepository] fetch user recent conv list by version range success",
		"user_id", userID,
		"low_version", lowVersion,
		"up_version", upVersion,
		"count", len(convDataList))
