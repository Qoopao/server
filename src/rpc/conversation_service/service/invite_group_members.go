...
func (s *conversationServiceImpl) InviteGroupMembers(ctx context.Context, req *sdkws.InviteGroupMembersRequest) (*sdkws.InviteGroupMembersResponse, error) {
	...
	convMap, err := s.storage.BatchGetConversations(ctx, []string{convID}, "")
	if err != nil {
	...
	if err = s.storage.UpdateConversation(ctx, conv); err != nil {
		return &sdkws.InviteGroupMembersResponse{ErrorCode: 500, ErrorMsg: err.Error()}, nil
	}
...
func (s *conversationServiceImpl) buildInvitedToGroupCmd(ctx context.Context, conv *sdkws.ConversationData) *sdkws.CmdMessage {
	if conv == nil {
		return nil
	}
	data, err := conv.Marshal(nil)
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationService] buildInvitedToGroupCmd marshal conv failed: conv_id=%s err=%v", conv.ConvID, err)
		return nil
	}
