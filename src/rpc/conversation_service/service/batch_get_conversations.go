...
func (s *conversationServiceImpl) BatchGetConversations(ctx context.Context, req *sdkws.BatchGetConversationsRequest) (*sdkws.BatchGetConversationsResponse, error) {
	...
	// 调用 storage 层批量查询
	convMap, err := s.storage.BatchGetConversations(ctx, convIDs, ownerID)
	if err != nil {
		klog.CtxErrorf(ctx, "[ConversationService] batch get conversations failed",
			"conv_ids", convIDs,
			"owner_id", ownerID,
			"error", err.Error())
		return &sdkws.BatchGetConversationsResponse{
	...
	klog.CtxDebugf(ctx, "[ConversationService] batch get conversations success",
