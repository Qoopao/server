...
func (s *conversationStorageImpl) BatchGetConversations(ctx context.Context, convIDs []string, ownerID string) (map[string]*sdkws.ConversationData, error) {
	if len(convIDs) == 0 {
	...
	for _, doc := range docs {
		...
		if err := convData.Unmarshal(doc.Data); err != nil {
			klog.CtxErrorf(ctx, "[ConversationStorage] unmarshal conversation data failed",
				"conv_id", doc.ID,
				"error", err.Error())
			continue
		}
	...
	klog.CtxDebugf(ctx, "[ConversationStorage] batch get conversations success",
