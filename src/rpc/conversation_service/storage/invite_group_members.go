...
func (s *conversationStorageImpl) UpdateConversation(ctx context.Context, conv *sdkws.ConversationData) error {
	...
	if err := store.UpdateOne(ctx, orm.CollectionConversations, bson.M{"_id": conv.ConvID}, update); err != nil {
		klog.CtxErrorf(ctx, "[ConversationStorage] update conversation failed", "conv_id", conv.ConvID, "error", err.Error())
		return err
	}

	klog.CtxDebugf(ctx, "[ConversationStorage] update conversation success", "conv_id", conv.ConvID)
	return nil
}
