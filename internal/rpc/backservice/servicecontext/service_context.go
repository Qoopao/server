...
type ServiceContext interface {
	...
	// GetPushServiceClient 获取推送服务客户端（懒加载）
	GetPushServiceClient() (pushservice.Client, error)
...
// GetPushServiceClient 获取推送服务客户端（懒加载）
func (i *impl) GetPushServiceClient() (pushservice.Client, error) {
	serviceName := "push-service"
	client, err := i.getServiceClient(serviceName, func(hostPort string) (interface{}, error) {
		client, err := pushservice.NewClient(
			serviceName,
			client.WithHostPorts(hostPort),
			client.WithSuite(tracing.NewClientSuite()),
			client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: serviceName}),
	...
	return client.(pushservice.Client), nil
}
