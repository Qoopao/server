...
func NewWsServer( /*msgGatewayConfig *Config,*/ opts ...Option) *WsServer {
	...
	// OTEL 初始化在 main.go 中进行，不在构造函数中
	// 避免 defer 在函数返回时立即执行导致 provider 被关闭
...
func (ws *WsServer) wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if ws.onlineUserConnNum.Load() >= 1000 /* ws.wsMaxConnNum  */ {
	...
	wsLongConn := newGWebSocket(WebSocket, 1000, 1000)
